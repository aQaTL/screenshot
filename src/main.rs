...
use winapi::um::winuser::{GetDC, ReleaseDC, MB_ICONERROR, MessageBoxW};
...
fn main() {
	...
	let mut file = File::create("screenshot.bmp").unwrap();
	file.write(&*bmp).unwrap();
}
...
fn screenshot(rect: Option<Rect>) -> Result<Vec<u8>, WinApiError> { unsafe { screenshot_(rect) } }
